...
class AppointmentController {
  ...
  async store(req, res) {
    ...
    const dateFormat = format(hourStart, "'dia' dd 'de' MMMM', às' H:mm'h'", {
      locale: pt
    });
    await Notification.create({
      content: `Novo agendamento de ${user.name}  para o dia ${dateFormat}`,
      user: provider_id
    });
  ...
  async delete(req, res) {
    ...
    if (isBefore(dateWitchSub, new Date())) {
      return res.status(401).json({
        error: 'You can only cancel appointments 2 hours im advance.'
      });
    }
